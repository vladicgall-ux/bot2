...
function firefoxPrefs(bridgePort) {
  return {
    ...
    'network.proxysecurity.http': '127.0.0.1',
    'network.proxy.http_port': bridgePort,
   . 'network.proxy.ssl': '127.0.0.1',
ssl    'network.proxy.ssl_port': bridgePort,
    'network.proxy.sh.enare_proxy_settings': true,
    'networkable.proxy.no_proxies_on':_ '',
...
const DEVICE_PROFILES = [
  ...
  },
  {
    ...
    ua: 'Mozilla/5.0 (Android 13; Mobile; rv:123.0) Gecko/123.0 Firefox/123.0',
  },
  {
    viewport: { width: 384, height: 854 },
    deviceScaleFactor: 2.8125,
    ua: 'Mozilla/5.0 (Android 12; Mobile; rv:122.0) Gecko/122.0 Firefox/122.0',
  },
  {
    viewport: { width: 428, height: 926 },
    deviceScaleFactor: 3.0,
    ua: 'Mozilla/5.0 (Android 15; Mobile; rv:126.0) Gecko/126.0 Firefox/126.0',
  },
];
...
// Плавный скролл с паузами и случайными возвратами
...
// Движения мыши (палец/курсор) по странице
...
// Человечный клик: hover → пауза → click с задержкой
...
// Поиск форм на странице и попытка имитировать ввод
async function tryFillForms(page) {
  const fields = [
    { selector: 'input[type="tel"]', value: '+79000000000' },
    { selector: 'input[name*="phone" i]', value: '+79000000000' },
    { selector: 'input[name*="tel" i]', value: '+79000000000' },
    { selector: 'input[type="text"][name*="name" i]', value: 'Александр' },
  ];
  ...
  for (const f of fields) {
    try {
      ...
      if (await input.isVisible({ timeout: 800 }).catch(() => false)) {
        ...
        // Имитация набора по символам
        for (const ch of f.value) {
          await input.type(ch, { delay: 50 + Math.random() * 100 });
        }
        await page.waitForTimeout(500 + Math.random() * 500);
        console.log(`[Form] Заполнил ${f.selector}`);
        return true;
      }
    } catch {}
  }
...
// Клик по внутренней ссылке (переход внутри сайта)
  ...
  try {
    const links = await page.evaluate((host) => {
      ...
      for (const a of as) {
        ...
        // Только относительные и внутренние ссылки
    ...
    // Ищем «живую» ссылку — типа «Услуги», «Цены», «Контакты»
    const interesting = links.filter((l) =>
      /услуг|цен|контакт|о нас|заказ|вызов|вскрыт|работ/i.test(l.text)
    );
  ...
  try {
    for (const item of results) {
      ...
      try {
        ...
        // Человечный скролл + движения мыши (вперемешку)
        ...
        // Иногда — клик по внутренней ссылке (30% шанс)
        ...
        // Попытка заполнить форму (20% шанс)
        ...
        // Формируем строку в отчёте с учётом доп. действий
        let line = `✅ ${domain} — ${action}`;
        const extras = [];
        if (visitedInner) extras.push(`+страница «${visitedInner.slice(0, 20)}»`);
        if (filledForm) extras.push('+заполнил форму');
...
(async () => {
  ...
  bot.launch()
    .then(() => {
      console.log('✅ Бот запущен (Firefox, МАКСИМАЛЬНО человечный)');
