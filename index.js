...
const DEVICE_PROFILES = [
  { viewport: { width: 412, height: 915 }, deviceScaleFactor: 2.625, ua: 'Mozilla/5.0 (Android 14; Mobile; rv:124.0) Gecko/124.0 Firefox/124.0' },
  { viewport: { width: 393, height: 851 }, deviceScaleFactor: 2.75, ua: 'Mozilla/5.0 (Android 14; Mobile; rv:125.0) Gecko/125.0 Firefox/125.0' },
  { viewport: { width: 360, height: 800 }, deviceScaleFactor: 3.0, ua: 'Mozilla/5.0 (Android 13; Mobile; rv:123.0) Gecko/123.0 Firefox/123.0' },
  { viewport: { width: 384, height: 854 }, deviceScaleFactor: 2.8125, ua: 'Mozilla/5.0 (Android 12; Mobile; rv:122.0) Gecko/122.0 Firefox/122.0' },
  { viewport: { width: 428, height: 926 }, deviceScaleFactor: 3.0, ua: 'Mozilla/5.0 (Android 15; Mobile; rv:126.0) Gecko/126.0 Firefox/126.0' },
];
  ...
  for (const sel of nameSelectors) {
    try {
      ...
      if (await inp.isVisible({ timeout: 500 }).catch(() => false)) {
        ...
        console.log(`[Form] Имя заполнено: ${NAME}`);
        filled = true;
  ...
  for (const sel of phoneSelectors) {
    try {
      ...
      if (await inp.isVisible({ timeout: 500 }).catch(() => false)) {
        ...
        console.log(`[Form] Телефон заполнен: ${PHONE}`);
        filled = true;
        await page.waitForTimeout(300 + Math.random() * 300);
        break;
      }
  ...
  for (const sel of reasonSelectors) {
    try {
      ...
      if (await inp.isVisible({ timeout: 500 }).catch(() => false)) {
        ...
        console.log(`[Form] Причина заполнена: ${REASON}`);
        filled = true;
        await page.waitForTimeout(300 + Math.random() * 300);
        break;
      }
  ...
  try {
    for (const item of results) {
      ...
      try {
        ...
        // Клик по CTA/tel ПЕРЕД заполнением формы — чтобы открылось модальное окно
        let action = 'просмотр';
        ...
        // Заполняем форму после клика по CTA (если открылось модальное окно)
        ...
        if (filledForm) extras.push('+заполнил форму (Николай, 8908073211)');
