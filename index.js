...
// ─── Расширенная маскировка для Firefox ─────────────────────────────────────
  ...
  try {
    browser = await firefox.launch({
      ...
      firefoxUserPrefs: {
        ...
        'network.proxy.http_port': parseInt(LOCAL_PROXY_URL.split(':').pop()),
        'network.proxy.ssl': '127.0.0.1',
        'network.proxy.ssl_port': parseInt(LOCAL_PROXY_URL.split(':').pop()),
...
async function searchYandexFirefox(query, ctx) {
  ...
  try {
    ...
    for (const url of targets) {
      try {
        console.log('[Firefox] Открываю', url);
        await page.goto(url, { waitUntil: 'domcontentloaded', timeout: 45000 });
        console.log('[Firefox] OK:', url);
        opened = true;
        break;
      } catch (e) {
        console.warn('[Firefox] Не открылся', url, ':', e.message.split('\n')[0]);
      }
    }
