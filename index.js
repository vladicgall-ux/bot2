...
bot.start((ctx) => {
  ctx.reply(
    `🤖 Бот серфинга (Firefox + Bridge, ПОЛНЫЙ)\n\n` +
...
// ─── СЕРФИНГ (ПОЛНЫЙ: поиск + обход сайтов через Firefox с кликами) ─────────
async function runSurf(query, stopSet, ctx) {
  ...
  await ctx.reply(`📋 Найдено: ${results.length} сайтов. Обход через Firefox (клики)...`);

  // Обход сайтов через Firefox с реальными действиями
  const bridgePort = parseInt(LOCAL_PROXY_URL.split(':').pop());
  ...
  try {
    for (const item of results) {
      ...
      try {
        ...
        // ─── Скроллинг 8–12 секунд ───────────────────────────────────
        const scrollTime = 8000 + Math.random() * 4000;
        const st = Date.now();
        while (Date.now() - st < scrollTime) {
          await page.evaluate((y) => window.scrollBy({ top: y, behavior: 'smooth' }), 200 + Math.random() * 300);
          await page.waitForTimeout(500 + Math.random() * 500);
        }

        // ─── Пробуем кликнуть tel: ────────────────────────────────────
        let action = 'просмотр';
        ...
        console.log(`[Visit] ${domain}: ${err.message}`);
...
// ─── ПОИСК ЧЕРЕЗ FIREFOX (прямой URL выдачи) ────────────────────────────────
...
(async () => {
  ...
  bot.launch()
    .then(() => {
      console.log('✅ Бот запущен (Firefox, ПОЛНЫЙ режим)');
