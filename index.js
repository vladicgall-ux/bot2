...
async function startLocalBridge() {
  ...
  if (LOCAL_PROXY_SERVER) {
    try { await LOCAL_PROXY_SERVER.close(true); } catch {}
  ...
  try {
    LOCAL_PROXY_SERVER = await ProxyChain.Server({
      port: 0, // случайный свободный порт
      verbose: false,
      prepareRequestFunction: () => ({
        upstreamProxyUrl: upstream,
        // Не форвардим авторизацию наверх — ProxyChain сам её добавит
        requestAuthentication: false,
      }),
    });

    // Ждём, пока сервер получит порт
    await new Promise((resolve) => setTimeout(resolve, 500));

    const addr = LOCAL_PROXY_SERVER.address;
    const port = typeof addr === 'object' ? addr.port : LOCAL_PROXY_SERVER.port;

    LOCAL_PROXY_URL = `http://127.0.0.1:${port}`;
...
async function changeProxyIP(ctx = null, force = false) {
  ...
  // Пересоздаём локальный мост с новым паролем
...
bot.start((ctx) => {
  ctx.reply(
    `🤖 Бот серфинга (Playwright + Bridge)\n\n` +
...
bot.command('testproxy', async (ctx) => {
  await ctx.reply('🧪 Диагностика через локальный мост...');

  if (!LOCAL_PROXY_URL) {
    await startLocalBridge();
  }
  if (!LOCAL_PROXY_URL) {
    return ctx.reply('❌ Не удалось поднять локальный мост.');
  }
  ...
  try {
    ...
    await ctx.reply(`❌ Ошибка Playwright: ${e.message}`);
  } finally {
...
async function runSurf(query, stopSet, ctx) {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      results = await searchYandexPlaywright(query, ctx);
      break;
    } catch (err) {
...
// ─── ПОИСК ЧЕРЕЗ PLAYWRIGHT + ЛОКАЛЬНЫЙ МОСТ ────────────────────────────────
async function searchYandexPlaywright(query, ctx) {
  if (!LOCAL_PROXY_URL) {
    throw new Error('Локальный мост не поднят');
  }
...
process.once('SIGINT', async () => {
  stopScheduler();
  if (LOCAL_PROXY_SERVER) await LOCAL_PROXY_SERVER.close(true).catch(() => {});
  bot.stop('SIGINT');
});
