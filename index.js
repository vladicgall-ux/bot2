...
bot.command('testproxy', async (ctx) => {
  await ctx.reply('🧪 Тестирую прокси через Playwright...');
  const sites = [
  ...
  try {
    ...
    for (const url of sites) {
      ...
      try {
        ...
        lines.push(`❌ ${url} — ${err.message.split('\n')[0].slice(0, 60)}`);
      }
    }
...
bot.command('dumphtml', async (ctx) => {
  const dump = path.join(DATA_DIR, 'last.html');
  if (!fs.existsSync(dump)) return ctx.reply('❌ Файл пуст. Сначала /run.');
...
async function searchYandexPlaywright(query, proxy, ctx) {
  ...
  try {
    ...
    try { fs.writeFileSync(path.join(DATA_DIR, 'last.html'), html, 'utf8'); } catch {}
