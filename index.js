...
// ─── ЛОКАЛЬНЫЙ ПРОКСИ-МОСТ (anonymizeProxy) ────────────────────────────────
let LOCAL_PROXY_URL = null;
let LOCAL_PROXY_SERVER = null;

async function startLocalBridge() {
  // Закрываем предыдущий мост
  if (LOCAL_PROXY_SERVER) {
    try {
      await ProxyChain.closeAnonymizedProxy(LOCAL_PROXY_SERVER, true);
    } catch (e) {
      console.warn('[Bridge] Ошибка закрытия старого моста:', e.message);
    }
...
async function changeProxyIP(ctx = null, force = false) {
  ...
  // Пересоздаём мост с новым паролем
...
// ─── ПОИСК ЯНДЕКС ЧЕРЕЗ PLAYWRIGHT + МОСТ ──────────────────────────────────
